<template>
  <div class="ressources-container">
    ...
    <div class="d-flex justify-space-between align-center mb-6">
      <div>
        <h1 class="text-h4 font-weight-bold">Ressources</h1>
        <p class="text-grey-darken-1">Gestion complète des ressources</p>
      </div>
      <v-btn
        color="primary"
        size="large"
    ...
    <v-card elevation="2" class="mb-6">
      <v-table hover>
        ...
        <tbody>
          ...
          <tr v-else-if="store.paginatedRessources.length === 0">
            <td :colspan="headers.length + 1" class="text-center py-8">
              <v-icon size="48" class="d-block mb-4 text-grey">mdi-inbox-outline</v-icon>
              <p class="text-grey">Aucune ressource trouvée</p>
            </td>
          </tr>
          <tr v-for="ressource in store.paginatedRessources" :key="ressource.id">
            <td>
              <span class="font-weight-bold text-primary">{{ ressource.id }}</span>
            </td>
            ...
            <td>
              <div class="d-flex gap-2">
                <v-btn
                  ...
                  color="info"
                  variant="text"
                  @click="openEditDialog(ressource)"
                  :loading="store.loading"
                  :disabled="!permission.canUpdate"
                >
                  <v-icon>mdi-pencil-outline</v-icon>
                  <v-tooltip activator="parent" location="top">
                    {{ permission.canUpdate ? 'Modifier' : '⛔ Pas de permission' }}
                  </v-tooltip>
                </v-btn>
                <v-btn
                  icon
                  ...
                  color="error"
                  variant="text"
                  @click="openDeleteDialog(ressource)"
                  :loading="store.loading"
                  :disabled="!permission.canDelete"
                >
                  <v-icon>mdi-delete-outline</v-icon>
                  <v-tooltip activator="parent" location="top">
                    {{ permission.canDelete ? 'Supprimer' : '⛔ Pas de permission' }}
                  </v-tooltip>
                </v-btn>
              </div>
            </td>
        ...
        <p class="text-caption text-grey">
          ...
          {{
            Math.min(
              ...
              store.filteredRessources.length,
            )
          }}
          sur {{ store.filteredRessources.length }}
        </p>
        <v-pagination
  ...
  <v-dialog v-model="showFormDialog" max-width="600px" persistent>
    <v-card>
      <v-card-title class="d-flex justify-space-between align-center">
        <span>{{ isEditing ? 'Modifier la Ressource' : 'Créer une Ressource' }}</span>
    ...
    <v-card>
      ...
      <v-card-text class="pa-6">
        <p>
          ...
          <strong>{{ selectedRessource?.name }}</strong> ?
        </p>
...
const openEditDialog = (ressource) => {
...
const submitForm = async () => {
  try {
    if (isEditing.value) {
      await store.update(selectedRessource.value.id, formData.value)
      notifySuccess('Ressource mise à jour avec succès!')
    } else {
      await store.createNew(formData.value)
      notifySuccess('Ressource créée avec succès!')
    }
    closeFormDialog()
  } catch (err) {
...
const openDeleteDialog = (ressource) => {
  selectedRessource.value = ressource
...
const confirmDelete = async () => {
  try {
    await store.remove(selectedRessource.value.id)
    notifySuccess(`Ressource "${selectedRessource.value.name}" supprimée avec succès!`)
